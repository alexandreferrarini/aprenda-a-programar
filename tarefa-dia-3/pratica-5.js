...
let valorTotalsemGorjeta = 0;
let valorTotalcomGorjeta = 0;
let totalVendasAbaixo50reais = 0;

function somarValores(item) {
    valorTotalsemGorjeta+= item.valor;   
    valorTotalcomGorjeta+= item.valor + item.gorjeta;   
    if (item.valor < 50) {
        totalVendasAbaixo50reais++;     
    }    
}

receita.forEach(somarValores);

/* PROBLEMA 1: Qual o valor total de vendas sem gorjeta? --------------------- */
//COLOQUE SUA SOLUÇÃO AQUI

console.log(`\nValor total sem gorjeta: ${valorTotalsemGorjeta}`);
...
console.log(`\nValor total com gorjeta: ${valorTotalcomGorjeta}`);
...
console.log(`\nTotal de vendas abaixo de 50 reais: ${totalVendasAbaixo50reais}`);
